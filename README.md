...
## Building
1. You need [Go 1.21 or newer](https://go.dev/dl/).
2. Clone git repo.
3. `go build`
...
```
$ ./pixiedust -in unifi-fresh-usw.pcap -findkeys -message=false -log-level=error
...
time=2026-08-12T09:00:00.000-06:00 level=INFO msg="discovered key" auth_key=0ee876dee74ff09c2e88387ecda39512 device=10.0.1.48
{
  "architecture": "armv7l",
  "board_rev": 9,
  "bootid": 0,
...
Diagnostic logs are written to standard error. Use `-log-level` to select the
minimum level (`debug`, `info`, `warn`, or `error`); the default is `info`.
